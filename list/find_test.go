...
func ExampleFind() {
	r, err := list.Find(mod2, []int{1, 2, 3, 4})
	if err != nil {
		panic(err)
	}
	fmt.Println(r)
...
func Example2Find() {
	_, err := list.Find(mod6, []int{1, 2, 3})
	fmt.Println(err)
	// Output: key not found
}

func ExampleFindBack() {
	r, err := list.FindBack(mod2, []int{1, 2, 3, 4})
	if err != nil {
		panic(err)
	}
	fmt.Println(r)
...
func ExampleTryFind() {
	r := list.TryFind(mod2, []int{1, 2, 3, 4})
	if r.IsNone() {
...
func ExampleTryFindBack() {
	r := list.TryFindBack(mod2, []int{1, 2, 3, 4})
	if r.IsNone() {
